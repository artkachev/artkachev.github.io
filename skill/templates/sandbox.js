(function () {
  ...
  function sheet() {
    ...
    lab.appendChild(chk);
    lab.appendChild(el("span", null, "убрать с витрины"));
    f3.appendChild(lab);
    f3.appendChild(el("p", "hint",
      "Работа пропадёт только с главной страницы. В каталоге всех работ, на " +
      "странице артиста и в карте сайта она останется, и своя страница у неё " +
      "собирается как обычно — из поиска люди на неё приходят по-прежнему."));
    card.appendChild(f3);
  ...
  function viewSum() {
    ...
    Object.keys(S.edits).forEach(function (slug) {
      ...
      if (typeof e.hidden === "boolean") {
        c.appendChild(el("p", null, e.hidden ? "убрать с витрины" : "вернуть на витрину"));
      }
      box.appendChild(c);
    });
